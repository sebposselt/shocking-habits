...
exports.itr = function (payload) {
    ...
    for (let i = 0; i < (payload.features).length; i++) {
        ...
        let tmp = new this.QuakeArticlesCont(paramConst(elm));
        returnArr.push(tmp);
    }
    return returnArr;
}
...
exports.QuakeArticlesCont = function (earthquakeObj){
    this.articles = [];
    this.earthquakeObj = earthquakeObj;
    
    // function to fetch articles
    this.getArticles = function(page) {
        let pageNumber = page;
        let quake = this.earthquakeObj;
