...
exports.JsonToMarker = function (obj, articles_2d) {
    var res = []
    console.log("TYPEOF",articles_2d);
    for (let i = 0; i < obj.features.length; i++) {
        var tmp = {};
        var elm = obj.features[i];
        var articles = articles_2d[i]
        
        tmp.coords = { lat: elm.properties.lat, lng: elm.properties.lon };
        tmp.content = "<p>"; //begin html
        tmp.content = tmp.content +  "Magnitude: " +  String(elm.properties.mag) + "<br>";
        tmp.content = tmp.content +  "Date: " +  String(elm.properties.time) + "<br>"; 

        if (articles) {
            tmp.content = tmp.content + "Score: " + parser.score(articles);
            // for (let j = 0; j < articles.length; j++) {
            //     var article = articles[j];
            //     tmp.content = tmp.content + "<br \> <p> Author:" + article.author + "</p>"
            // }
        }
